// nt-mt4bind.cpp : Defines the exported functions for the DLL application.
//
...
EXPORT int WINAPI _zmq_get_opt_more (void*  socket) {
  ...
  zmq_getsockopt (socket, ZMQ_RCVMORE, &more, reinterpret_cast<size_t*>(&more_size));
  return more; 
}
...
{
  void* ret =  malloc(sizeof(zmq_msg_t));  return reinterpret_cast<zmq_msg_t*>(ret);
}  
